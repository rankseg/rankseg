"""Check release README rendering and the files needed to test an sdist.
...
def check_distributions(dist_dir):
    ...
    with tarfile.open(sdists[0], "r:gz") as sdist:
        ...
        required = [root / "tests/conftest.py", *sorted((root / "tests").glob("test*.py"))]
    ...
    print(f"RankSEG {version}: PyPI README rendering, release image URLs, and sdist test files verified")
