...
def rank_dice(output, device, app=2, smooth=0.0, allow_overlap=True, truncate_mean=False, pruning=True, verbose=0):
    """
    ...
    if truncate_mean == True:
        pb_mean = 1.0 * (sorted_prob >= 0.5).sum(axis=-1)
    else:
    ...
    #                                     pb_var=pb_var,
    #                                     dim=dim)
    low_class, up_class = app_action_set(pb_mean=pb_mean, pb_var=pb_var, pb_m3=pb_m3, device=device, dim=dim)

    for k in range(num_class):
        ## searching for optimal vol for each sample and each class
        for b in range(batch_size):
            if (sorted_prob[b, k, 0] <= 0.5) and pruning:
            ...
            if app_tmp > 1:
                pmf_tmp = PB_RNA(
                    pb_mean[b, k], pb_var[b, k], pb_m3[b, k], device=device, up=up_class[b, k], low=low_class[b, k]
                )
                ...
                ma_tmp = F.conv1d(
                    (2.0 / (discount[low_tmp:up_tmp] + smooth + 2)).view(1, 1, -1), pmf_tmp.view(1, 1, -1)
                )
                nu_range = F.conv1d(
                    (smooth / (discount[low_tmp:up_tmp] + smooth + 1)).view(1, 1, -1), pmf_tmp.view(1, 1, -1)
                )
                ...
                score_zero = smooth * torch.sum((1.0 / (discount[low_class[b, k] : up_class[b, k]] + smooth)) * pmf_tmp)
                if best_score <= score_zero:
                    best_score = score_zero
                    opt_tau = 0
                if verbose == 1:
                    print(
                        "TEST sample-%d; class-%d; mean_pb: %.1f; up_tau:%d; tau_best: %d; score_best: %.4f"
                        % (b, k, pb_mean[b, k], up_tau[b, k], opt_tau, best_score)
                    )
            if app_tmp <= 1:
                ...
                if smooth > 0:
                    best_score = smooth * torch.sum(
                        (1.0 / (discount[low_class[b, k] : up_class[b, k]] + smooth)) * pmf_tmp_zero
                    )
                ...
                for tau in range(1, up_tau[b, k] + 1):
                    ...
                    pmf_tmp = PB_RNA(
                        pb_mean_tmp, pb_var_tmp, pb_m3_tmp, device=device, up=up_class[b, k], low=low_class[b, k]
                    )

                    pmf_tmp = pmf_tmp / torch.sum(pmf_tmp)
                    w_old = w_old + sorted_prob[b, k, tau - 1] * pmf_tmp
                    omega_tmp = torch.sum(2.0 / (discount[low_class[b, k] : up_class[b, k]] + tau + smooth + 2) * w_old)
                    nu_tmp = smooth * torch.sum(
                        (1.0 / (discount[low_class[b, k] : up_class[b, k]] + tau + smooth + 1)) * pmf_tmp_zero
                    )
                ...
                if verbose == 1:
                    ...
                    print(
                        "sample-%d; class-%d; mean_tau: %d; up_tau: %d; tau_best: %d; score_best: %.4f"
                        % (b, k, int(pb_mean[b, k]), up_tau[b, k], opt_tau, best_score)
                    )
            predict[b, k, top_index[b, k, :opt_tau]] = True
            tau_rd[b, k] = opt_tau
            cutpoint_rd[b, k] = sorted_prob[b, k, opt_tau]
...
def PB_RNA(pb_mean, pb_var, pb_m3, device, up, low=0, top=None):
    ...
    cdf_tmp = norm_rv.cdf(candidate_val) + pb_m3 * (1 - candidate_val**2) * torch.exp(
        norm_rv.log_prob(candidate_val)
    ) / 6 / pb_var ** (3 / 2)
    ...
    if top != None:
        if top < (up - low):
            ...
            pmf_tmp[pmf_tmp < split_val] = float(0.0)
